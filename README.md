...
Este projeto faz parte da Situação de Aprendizagem do curso de Programação Back-End do SENAI. O módulo é responsável por enviar notificações (confirmação de inscrição, lembretes) para participantes de eventos.
...
```bash
git add .
git commit -m "Bloco 1 completo: MVC + Swagger + Postman"
git push